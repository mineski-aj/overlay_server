...
/* The standings API reports each week's medal separately
   ...
   'gold'/'silver' by walking week 1 → 8, so mplfs.html's Standings scene
   can render medals left-to-right in the order they were actually earned
   instead of grouping all golds before all silvers. */
function buildWeeklyMedals(row) {
  const medals = [];
  for (let wk = 1; wk <= 8; wk++) {
    if (Number(row[`gold_wk_${wk}`]))   medals.push('gold');
    if (Number(row[`silver_wk_${wk}`])) medals.push('silver');
  }
  return medals;
}
