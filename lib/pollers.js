...
function getGameApiUrl() {
  const url = readUrlForMode(GAME_URL_FILE, '').trim();
  return url || GAME_API;
}
...
function getPostInfoUrl() {
  const url = readUrlForMode(POST_INFO_URL_FILE, '').trim();
  return url || POST_INFO_API_DEFAULT;
}
      ...
      try {
        ...
        if (apiState === "play" && !paused) {
          ...
          for (const camp of camps) {
            ...
            // No acting player for this one — instead pass the camp of the
            // OTHER team (the one that did the wiping), so 10th Anniversary
            // can show that team's logo (see killEventTeamLogoSrc in
            // overlay-killevents.js). Regular theme ignores this camp value.
            const allDead = [1,2,3,4,5].every(s => camp[`seat_${s}`] && camp[`seat_${s}`].dead === true);
            if (allDead && !state.prevKillEventCounts[`wipeout_${cid}`]) {
              const killerCamp = cid === 1 ? 'red' : 'blue';
              broadcastKillEvent('wipedout.webm', 3, null, null, null, killerCamp);
            }
          ...
          for (const camp of camps) {
            ...
            for (let s = 1; s <= 5; s++) {
              ...
              state.positionLog.push({
                ...
                camp: camp.campid,
                seat: s,
                x:           seat.map_pos ? seat.map_pos.x : null,
                y:           seat.map_pos ? seat.map_pos.y : null,
                gold:        seat.gold        ?? null,
...
function getStandingsApiUrl() {
  const url = readUrlForMode(STANDINGS_URL_FILE, '').trim();
  return url || STANDINGS_API_DEFAULT;
}
...
/* The standings API reports each week's medal separately
   (gold_wk_1..gold_wk_8 / silver_wk_1..silver_wk_8, one flag per week per
   team) rather than a single running total — build an ORDERED list of
   { type: 'gold'|'silver', wk } by walking week 1 → 8, so mplfs.html's
   Standings scene can render medals left-to-right in the order they were
   actually earned (instead of grouping all golds before all silvers) and
   label each medal with the week it was earned. */
function buildWeeklyMedals(row) {
  const medals = [];
  for (let wk = 1; wk <= 8; wk++) {
    if (Number(row[`gold_wk_${wk}`]))   medals.push({ type: 'gold', wk });
    if (Number(row[`silver_wk_${wk}`])) medals.push({ type: 'silver', wk });
  }
  return medals;
}
