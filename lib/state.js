...
const state = {
  ...
  mplfsScene: { matchboard: false, middleboard: false, playerboard: false, activeFeature: null, scheduleHighlight: 0 },
  // Consolidated Post 2 — which of its two pages is currently shown
  // (1 = Runes/Items, 2 = Damage Dealt/Taken), scene-wide across all 10
  // rows. See routes/overlay.js's /overlay/cp2page/:page and
  // /overlay/cp2-page (restore-on-load).
  cp2Page: 1,
  ...
  // html/mpltag.html — Player H2H panel. role is one of
  // gold/jungler/exp/mid/roamer, or null when hidden. Only one role is
  // ever shown at a time — switching roles animates the current one out
  // before animating the new one in (see routes/overlay.js).
  playerh2h: { role: null },
  featureToggles: {
    killevents: true,
    items:      true,
    ...
    draftstats: true, // Draft.html pick reveal: PICK / CONTENTION RATE / WIN RATE (blue side only for now)
    draftpredict_rationale: true, // Draft Predict AI text bubble — off = candidate carousels only, no rationale bubble
    lineupsecrole_blue: true, // Team Line-Up (mplfs) Blue: whether the Secondary Role plate reveals after the pose settles
    lineupsecrole_red:  true, // same, Red side
    stolengold: true, // Jungle Resource Stolen banner — auto-fires at 2/5/8/12min game time
  },
  prevKillEventCounts: {},
  ...
  checkOverlays: {
    ...
    emblemcheck:     false,
    golddiffcheck:   false,
    goldgraphcheck:  false,
    sideexpcheck:    false,
    sidetakencheck:  false,
    sidedamagecheck: false,
    sidegoldcheck:   false,
    sidegolddistricheck: false,
    sidekdadistricheck: false,
    draftrecap:      false,
    draftpredict:    false,
    bpmmeter:        false,
    bpmmeter_tag:    false,
