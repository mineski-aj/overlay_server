...
// Kept for manual overrides — the simulator drives readings on its own, but
// you can still POST a one-off value to a slot (e.g. to sanity-check a single
// player's number) the same way a real watch would.
...
router.post('/bpm', (req, res) => {
  ...
  console.log(`[BPM] (manual override) player=${pid} bpm=${bpm}`);
  res.json({ ok: true });
});
